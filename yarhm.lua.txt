-- DRKscripts by Imperial, version 1.8.14

-- Instances:

local Converted = {
	["_DRKscriprs"] = Instance.new("ScreenGui");
...
Converted["_DRKscripts"].DisplayOrder = 999999999
Converted["_DRKscripts"].IgnoreGuiInset = true
Converted["_DRKscripts"].ScreenInsets = Enum.ScreenInsets.DeviceSafeInsets
Converted["_DRKscripts"].ResetOnSpawn = false
Converted["_DRKscripts"].ZIndexBehavior = Enum.ZIndexBehavior.Sibling
Converted["_DRKscripts"].Name = "DRKscripts"
Converted["_DRKscripts"].Parent = game:GetService("CoreGui")

Converted["_Open"].Font = Enum.Font.Gotham
Converted["_Open"].Text = "Triple-click this region to open DRKscripts."
...
Converted["_Open"].Parent = Converted["_DRKscripts"]
...
Converted["_TextLabel1"].Text = "DRKscripts"
...
Converted["_ListButton"].Parent = Converted["_DRKscripts"]
...
Converted["_Notifications"].Parent = Converted["_DRKscripts"]
...
Converted["_TextBoxPlaceholder"].Parent = Converted["_DRKscripts"]
...
Converted["_FloatingButton"].Parent = Converted["_DRKscripts"]
...
Converted["_FloatingButtons"].Parent = Converted["_DRKscripts"]
...
Converted["_DroppedGunBGUI"].Parent = Converted["_DRKscripts"]
...
Converted["_AddCustomModule1"].Parent = Converted["_DRKscripts"]
...
do -- Fake Module: StarterGui.DRKscripts.FUNCTIONS
    local script = Instance.new("ModuleScript")
    script.Name = "FUNCTIONS"
    script.Parent = Converted["_DRKscripts"]
		...
		function loader(module)
			...
			for _, item in ipairs(module) do
				...
				elseif item["Type"] == "Button" then
					...
					button.MouseButton1Down:Connect(function()
						...
						task.spawn(function()
							...
							if not _G.DRKscripts.FloatingButtons:FindFirstChild(item["Args"][1]) then
		
		
								local newFloatingButton = _G.DRKscripts.FloatingButton:Clone()
								newFloatingButton.Parent = _G.DRKscripts.FloatingButtons
								...
								_G.DRKscripts.FloatingButtons:FindFirstChild(item["Args"][1]):Destroy()
							end
						end)
					end)
				...
				elseif item["Type"] == "Input" then
					local cloneinput = _G.DRKscripts.TextBoxPlaceholder:Clone()
		...
		function module.refreshlist()
			...
			for modulei, module in ipairs(_G.Modules) do
				local success, err = pcall(function()
					local listbutton = _G.DRKscripts.ListButton:Clone()
				...
				if not success then
					warn("[DRKscripts] Error while requiring " .. module.Name .. "!")
					warn(err)
				end
			end
		end
...
local function GICVX_fake_script() -- Fake Script: StarterGui.DRKscripts.Open.InitOpen
...
local function ZNDKBAA_fake_script() -- Fake Script: StarterGui.DRKscripts.Open.OnClick
    local script = Instance.new("LocalScript")
    script.Name = "OnClick"
    script.Parent = Converted["_Open"]
    local req = require
	...
	script.Parent.MouseButton1Click:Connect(function()
		...
		if clickCount == 3 then
			...
			ts:Create(_G.DRKscripts.Menu, TweenInfo.new(0.7, Enum.EasingStyle.Back, Enum.EasingDirection.Out), 
...
local function VNSWJ_fake_script() -- Fake Script: StarterGui.DRKscripts.Menu.List.AutoSetup
    local script = Instance.new("LocalScript")
...
local function TTVRS_fake_script() -- Fake Script: StarterGui.DRKscripts.Menu.Close.LocalScript
    local script = Instance.new("LocalScript")
	...
	script.Parent.MouseButton1Click:Connect(function()
		ts:Create(_G.DRKscripts.Menu, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), 
			{Position = UDim2.fromScale(0.499, 0), Size = UDim2.fromOffset(441, 0)}
		):Play()
	end)
end
local function TZEK_fake_script() -- Fake Script: StarterGui.DRKscripts.Menu.AddCustomModule.LocalScript
    local script = Instance.new("LocalScript")
    script.Name = "LocalScript"
...
local function GIGHQO_fake_script() -- Fake Script: StarterGui.DRKscripts.Init
    local script = Instance.new("LocalScript")
    script.Name = "Init"
    script.Parent = Converted["_DRKscripts"]
    local req = require
	...
	require(script.Parent.FUNCTIONS).notification("Thanks for using DRKscripts! To use this hub, triple-click/tap the top region of your screen.")
...
local function RMKEEKN_fake_script() -- Fake Script: StarterGui.DRKscripts.Flee the Facility
    local script = Instance.new("LocalScript")
...
local function CSGP_fake_script() -- Fake Script: StarterGui.DRKscripts.Universal
    local script = Instance.new("LocalScript")
	...
	module[4] = {
		Type = "Button",
		Args = {"Move DRKscripts trigger to the bottom", function(Self)
			_G.DRKscripts.Open.UIStroke.Transparency = 0
			_G.DRKscripts.Open.TextTransparency = 0
			ts:Create(_G.DRKscripts.Open, TweenInfo.new(1, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {
				Position = UDim2.fromScale(0.5, 0.903)
			}):Play()
			
			ts:Create(_G.DRKscripts.Open.UIStroke, TweenInfo.new(1), {
				Transparency = 1
			}):Play()
			ts:Create(_G.DRKscripts.Open, TweenInfo.new(1), {
				TextTransparency = 1
			}):Play()
		end}
	}
	...
	module[5] = {
		Type = "Button",
		Args = {"Move DRKscripts trigger to the top", function(Self)
			_G.DRKscripts.Open.UIStroke.Transparency = 0
			_G.DRKscripts.Open.TextTransparency = 0
			ts:Create(_G.DRKscripts.Open, TweenInfo.new(1, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {
				Position = UDim2.fromScale(0.5, 0.063)
...
local function LOAOODV_fake_script() -- Fake Script: StarterGui.DRKscripts.Murder Mystery 2
    local script = Instance.new("LocalScript")
...
local function INPG_fake_script() -- Fake Script: StarterGui.DRKscripts.AddCustomModule.Add.LocalScript
    local script = Instance.new("LocalScript")
    script.Name = "LocalScript"
...
local function BOCLRJC_fake_script() -- Fake Script: StarterGui.DRKscripts.AddCustomModule.Cancel.LocalScript
    local script = Instance.new("LocalScript")
    script.Name = "LocalScript"
